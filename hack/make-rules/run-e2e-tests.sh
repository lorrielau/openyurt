...
#    set kubeconfig
    ...
    docker cp $KUBECONFIG $edgeNodeContainerName:$KUBECONFIG
}
...
function run_non_edge_autonomy_e2e_tests {
    ...
    pwd
    # ginkgo --gcflags "${gcflags:-}" ${goflags} --outputdir $filename "$YURT_ROOT/${testpkg}" --label-filter="!edge_autonomy" -r
...
#  run a nginx pod as static pod on each edge node
    local nginxYamlPath="${YURT_ROOT}/charts/openyurt/templates/nginx.yaml"
    local staticPodPath="/etc/kubernetes/manifests/"
    local POD_CREATE_TIMEOUT=60s

    # create service for nginx pods
    kubectl apply -f /etc/openyurt/charts/openyurt/templates/nginxService.yaml
